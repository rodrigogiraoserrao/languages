import sys;C="+-><,.[]";M=256;sys.argv.extend([""]*1) # useful constants and default arguments
def I(b,t=[0],p=0):
 while b: # interpret while there's code
  ...
  if c==4:i=sys.stdin.read(1)or"\u0000";t[p]=ord(i)%M # read one char as numeric input
  if c==5:sys.stdout.write(chr(t[p])) # print one char as output
  if c==6:
   d=1;j=[d:=d+(x=="[")-(x=="]")for x in b].index(0);b,b_=b[j+1:],b[:j]
   while t[p]:_,t,p=I(b_,t,p) # loop while memory cell is non-zero
 return b,t,p
b,t,p=I(sys.argv[1]);print();print(t,p) # interpret and print debugging info
